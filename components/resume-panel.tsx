 ...
 * Shows a stats grid (words, pages, format) after successful upload.
 */
export default function ResumePanel({
  ...
  const wordCount = pdf.text.trim() ? pdf.text.trim().split(/\s+/).length : 0;
  ...
  return (
    <div className="flex flex-col gap-2.5 rounded-xl border border-border-light bg-surface p-3.5 shadow-[var(--shadow-card)] max-[480px]:p-3">
            ...
            <div>
              <div className="flex items-center gap-2 rounded-lg bg-score-green-bg px-3 py-2.5">
                <div className="flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-[5px] bg-score-green-mid">
                  <svg className="h-[11px] w-[11px]" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
                ...
                <button
                  ...
                  className="cursor-pointer text-sm text-score-green-text/55 hover:text-score-green-text"
                >
                  ✕
                </button>
              </div>
              ...
              {pdf.text && !pdf.extracting && (
                <div className="mt-1 grid grid-cols-3 gap-2">
                  <div className="rounded-lg bg-surface-2 px-2.5 py-2 text-center">
                    <div className="text-[13px] font-bold text-foreground">{wordCount.toLocaleString()}</div>
                  ...
                  <div className="rounded-lg bg-surface-2 px-2.5 py-2 text-center">
                    <div className="text-[13px] font-bold text-foreground">PDF</div>
                    <div className="text-[10px] text-faint">Format</div>
                  </div>
                </div>
              )}
            </div>
      ...
      {activeTab === "paste" && (
        <div>
          <textarea
            className="h-[180px] w-full resize-y rounded-lg border border-border-light bg-surface-2 px-3 py-2.5 text-[13px] leading-relaxed text-foreground outline-none placeholder:text-faint focus:border-sky/40 max-[480px]:h-[140px]"
